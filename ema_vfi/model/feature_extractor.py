...
class InterFrameAttention(nn.Module):
    ...
    def forward(self, x1, x2, cor, H, W, mask=None):
        ...
        cor_embed = cor_embed_.reshape(
            B, N, self.num_heads, self.motion_dim // self.num_heads
        ).permute(0, 2, 1, 3)
        k, v = kv[0], kv[1]
    ...
    def forward(self, x, cor, H, W, B):
        ...
        if self.shift_size[0] or self.shift_size[1]:
            ...
            if hasattr(self, "HW") and self.HW.item() == H_p * W_p:
                ...
                mask_windows = window_partition(shift_mask, self.window_size).squeeze(
                    -1
                )
                shift_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
                shift_mask = shift_mask.masked_fill(
                    shift_mask != 0, float(-100.0)
                ).masked_fill(shift_mask == 0, float(0.0))

                if mask is not None:
                    shift_mask = shift_mask.masked_fill(mask != 0, float(-100.0))
        ...
        if self.shift_size[0] or self.shift_size[1]:
            x_back_win = torch.roll(
                x_back_win, shifts=(self.shift_size[0], self.shift_size[1]), dims=(1, 2)
            )
            x_motion = torch.roll(
                x_motion, shifts=(self.shift_size[0], self.shift_size[1]), dims=(1, 2)
            )

        x = depad_if_needed(x_back_win, x.size(), self.window_size).view(
            2 * B, H * W, -1
        )
        x_motion = depad_if_needed(x_motion, cor.size(), self.window_size).view(
            2 * B, H * W, -1
        )
...
class MotionFormer(nn.Module):
        ...
        for i in range(self.num_stages):
            if i == 0:
                ...
                if i < self.conv_stages:
                    ...
                    if i == self.conv_stages:
                        patch_embed = CrossScalePatchEmbed(
                            embed_dims[:i], embed_dim=embed_dims[i]
                        )
                    else:
                    ...
                    block = nn.ModuleList(
                        [
                            MotionFormerBlock(
                                ...
                                shift_size=0
                                if (j % 2) == 0
                                else window_sizes[i - self.conv_stages] // 2,
    ...
    def get_cor(self, shape, device):
        ...
        if k not in self.cor:
            tenHorizontal = (
                torch.linspace(-1.0, 1.0, shape[2], device=device)
                .view(1, 1, 1, shape[2])
                .expand(shape[0], -1, shape[1], -1)
                .permute(0, 2, 3, 1)
            )
            tenVertical = (
                torch.linspace(-1.0, 1.0, shape[1], device=device)
                .view(1, 1, shape[1], 1)
                .expand(shape[0], -1, -1, shape[2])
                .permute(0, 2, 3, 1)
            )
